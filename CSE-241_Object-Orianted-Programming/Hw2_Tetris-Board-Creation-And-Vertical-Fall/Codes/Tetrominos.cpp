...
void Tetromino::rotate(const string &l_or_r) {  //rotate the tetromino
...
bool Tetromino::canFit(Tetromino &other, string l_or_r) {  // check if any perfect fit between 2 tetrominos
    bool fit = false;
    vector<int> holes;
...
inline vector<vector<char>> Tetromino::get_tetro() const{
    return tetro_vector;
}

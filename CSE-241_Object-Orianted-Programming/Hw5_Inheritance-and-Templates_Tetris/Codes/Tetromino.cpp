...
#pragma region  input&global
    bool tReader(const std::istream &a) {return true;}

    bool tReader(const std::ostream &a) {return true;}

    charimino str_to_charmino(string chrmino)
    {
    ...
    charimino input::take_input() const
    {
        ...
        while (!valid) // take tetromino types and save an char vector
        {
            ...
            {
                chariminos = str_to_charmino(input_string);
            }
        }
...
#pragma region Tetromino
    ...
    Tetromino::~Tetromino()
    {
        if (tetro_vector != nullptr)
        {
            for (int i = 0; i < 4; ++i)
            {
                if (tetro_vector[i] != nullptr)
                    delete[] tetro_vector[i];
            }
            delete[] tetro_vector;
        }
    }

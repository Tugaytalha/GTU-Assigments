...
#pragma region TetrisAdapter

template<typename T>
TetrisAdapter<T>::TetrisAdapter(int wid, int hig) :  AbstractTetris(wid, hig) {

    board_vec.resize(this->high());
    for (int i = 0; i < high(); ++i) {
        board_vec[i].resize(this->wide(), ' ');
    }    
}
...
template<typename T>
bool TetrisAdapter<T>::operator+=(const Tetromino& Tetro) {
    int tet_wide = 4;
    bool wide_flag = true;
    tetro = Tetro.type();
...
template<typename T>
bool TetrisAdapter<T>::operator>>(int number) {
    bool return_value;
    if(number < 0) {
...
template<typename T>
bool TetrisAdapter<T>::move_test(char LorRorD) const{
    bool can_move = true;
...
template<typename T>
void TetrisAdapter<T>::operator||(int index) {    
...
template<typename T>
void TetrisAdapter<T>::move_right(int index) {
    bool inner_recursive = false;  
    if (index < 0) {inner_recursive = true; index*=-1;}
...
template<typename T>
void TetrisAdapter<T>::move_left(int index) {
    bool inner_recursive = false;  
    if (index < 0) {inner_recursive = true; index*=-1;}
...
bool TetrisAdapter<T>::animate() {
    ...
    while(tReader(cin >> rotate_string >> rotate_no) && rotate_no < 0) cout << "Enter valid direction and count";
    //step 3
    cout << "move Which direction and How many times? " << endl;
    while(tReader(cin >> move_string >> move_no) && move_no < 0) cout << "Enter valid direction and count";
...
template<typename T>
bool TetrisAdapter<T>::fileanimate(int rotate_no, int move_no) {
    bool valid = true;
    Draw();
...
template<typename T>
bool TetrisAdapter<T>::Rotate(string LorR) {
...
template<typename T>
bool TetrisAdapter<T>::Rotate_right(int index, int minx, int miny, int maxx, int maxy) {
...
TetrisAdapter<T>::~TetrisAdapter() {
    ...
    // do nothing T will be deleted automaticly
    // and last_loc will be deleted by AbstractTetris's destructor (auto call)
}

template<typename T>
void TetrisAdapter<T>::writeToFile(const std::string& filename) const{
...
template<typename T>
void TetrisAdapter<T>::readFromFile(const std::string &filename) {
    int nRot, nMove, ccc;
    string tetType;
    std::fstream saveFile;
...
template<typename T>
std::pair<int, int> TetrisAdapter<T>::lastMove() const {

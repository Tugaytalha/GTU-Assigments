...
#pragma region TetrisVector

TetrisVector::TetrisVector(int wid, int hig) :  AbstractTetris(wid, hig) {
    vector<char> temp(this->wide(), ' ');
    board_vec.resize(this->high(), temp);    
}

vector<char> &TetrisVector::operator[](int i) {
    return board_vec[i];
}

const vector<char> &TetrisVector::operator[](const int i) const{
    return board_vec[i];
}
...
bool TetrisVector::operator+=(const Tetromino& Tetro) {
    int tet_wide = 4;
    bool wide_flag = true;
    tetro = Tetro.type();
...
bool TetrisVector::operator>>(int number) {
    bool return_value;
    if(number < 0) {
...
bool TetrisVector::move_test(char LorRorD) const{
    bool can_move = true;
...
void TetrisVector::operator||(int index) {    
...
void TetrisVector::move_right(int index) {
    bool inner_recursive = false;  
    if (index < 0) {inner_recursive = true; index*=-1;}
...
void TetrisVector::move_left(int index) {
    bool inner_recursive = false;  
    if (index < 0) {inner_recursive = true; index*=-1;}
...
bool TetrisVector::fileanimate(int rotate_no, int move_no) {
    bool valid = true;
    Draw();
...
bool TetrisVector::Rotate(string LorR) {
...
bool TetrisVector::Rotate_right(int index, int minx, int miny, int maxx, int maxy) {
...
TetrisVector::~TetrisVector() {
    ...
    // do nothing vector will be deleted automaticly
    // and last_loc will be deleted by AbstractTetris's destructor (auto call)
}

void TetrisVector::writeToFile(const std::string& filename) const{
...
void TetrisVector::readFromFile(const std::string &filename) {
...
std::pair<int, int> TetrisVector::lastMove() const {
